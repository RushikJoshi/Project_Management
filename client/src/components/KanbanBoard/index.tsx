...
const COLUMNS: { id: TaskStatus; title: string; color: string }[] = [
  ...
  { id: 'todo', title: 'To Do', color: '#5e72a0' },
...
export const KanbanBoard: React.FC<KanbanBoardProps> = ({ projectId, onOpenTask, onAddTask }) => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (targetColumn && draggedTask.status !== targetColumn.id) {
      moveTask(draggedTask.id, targetColumn.id);
      return;
    }
    ...
    if (targetTask && targetTask.status !== draggedTask.status) {
      moveTask(draggedTask.id, targetTask.status);
      return;
    }
    ...
    if (currentColumn) {
      moveTask(draggedTask.id, currentColumn.id);
    }
  };

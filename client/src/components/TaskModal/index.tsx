                  ...
                  <div className="space-y-1.5">
                    {(task.subtasks || []).map(sub => (
                      <div key={sub.id} className="flex items-center gap-2 p-2.5 rounded-xl bg-surface-50 dark:bg-surface-800 border border-surface-100 dark:border-surface-700">
                        <input type="checkbox" checked={sub.status === 'done'} onChange={() => {}} className="rounded" />
                        <span className={cn('text-sm flex-1', sub.status === 'done' && 'line-through text-surface-400')}>{sub.title}</span>
                      </div>
                    ))}

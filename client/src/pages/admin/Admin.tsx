            ...
            { key: 'createdAt', header: 'Joined', render: (u) => <span className="text-xs text-surface-400">{formatDate(u.createdAt)}</span> },
            {
              ...
              render: (u) => (
                <div className="flex items-center gap-1 justify-end">
                  ...
                  <button className="btn-ghost btn-sm w-7 h-7 text-rose-400 hover:bg-rose-50"><Trash2 size={13} /></button>
                </div>
              )
            },
          ]}
        ...
        {selectedUser && (
          <div className="p-6 space-y-4">
            ...
            <div>
              ...
              <input defaultValue={selectedUser.jobTitle} className="input" />
            </div>
            <div>
              <label className="label">Department</label>
              <input defaultValue={selectedUser.department} className="input" />
            </div>
            <div className="flex items-center justify-between p-3 bg-surface-50 dark:bg-surface-800 rounded-xl">
              <div>
                ...
                <p className="text-xs text-surface-400">{selectedUser.isActive ? 'Currently active' : 'Account disabled'}</p>
              </div>
              <button className={cn('relative w-10 h-6 rounded-full transition-colors', selectedUser.isActive ? 'bg-brand-600' : 'bg-surface-200')}>
                <span className={cn('absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm transition-transform', selectedUser.isActive ? 'left-5' : 'left-1')} />
              </button>
            </div>
            <div className="flex gap-3 pt-2">
              <button onClick={() => setSelectedUser(null)} className="btn-secondary btn-md flex-1">Cancel</button>
              <button onClick={() => setSelectedUser(null)} className="btn-primary btn-md flex-1">Save changes</button>
            </div>
          </div>
        )}
      </Modal>
...
export const AdminPermissionsPage: React.FC = () => {
  const PERMISSIONS = [
    { label: 'Create Projects', description: 'Can create new projects in the workspace' },
    { label: 'Delete Projects', description: 'Can permanently delete projects' },
    { label: 'Manage Users', description: 'Can invite, edit, and remove users' },
    { label: 'View Reports', description: 'Access to analytics and reports' },
    { label: 'Manage Billing', description: 'Can view and update billing info' },
    { label: 'Export Data', description: 'Can export workspace data' },
    { label: 'Manage Settings', description: 'Can edit workspace-level settings' },
    { label: 'Create Teams', description: 'Can create and manage teams' },
  ];

  const ROLES: Role[] = ['super_admin', 'admin', 'manager', 'team_leader', 'team_member'];

  const DEFAULT_PERMISSIONS: Record<string, boolean[]> = {
    'Create Projects': [true, true, true, false, false],
    'Delete Projects': [true, true, false, false, false],
    'Manage Users': [true, true, false, false, false],
    'View Reports': [true, true, true, true, false],
    'Manage Billing': [true, true, false, false, false],
    'Export Data': [true, true, true, false, false],
    'Manage Settings': [true, true, false, false, false],
    'Create Teams': [true, true, true, false, false],
  };
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-surface-100 dark:border-surface-800 bg-surface-50 dark:bg-surface-800/50">
                <th className="text-left px-5 py-3 text-xs font-semibold text-surface-500 uppercase tracking-wider w-64">Permission</th>
                {ROLES.map(role => (
            ...
            <tbody className="divide-y divide-surface-50 dark:divide-surface-800/50">
              {PERMISSIONS.map((perm, pi) => (
                <motion.tr key={perm.label} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: pi * 0.04 }}
                  ...
                  {ROLES.map((role, ri) => {
                    const allowed = DEFAULT_PERMISSIONS[perm.label]?.[ri] ?? false;
                    return (
                      <td key={role} className="px-4 py-3.5 text-center">
                        <div className="flex items-center justify-center">
                          {allowed ? (
                            <div className="w-6 h-6 bg-emerald-50 dark:bg-emerald-950/30 rounded-full flex items-center justify-center">
                              <Check size={13} className="text-emerald-600" />
                            </div>
                          ) : (
                            <div className="w-6 h-6 bg-surface-100 dark:bg-surface-800 rounded-full flex items-center justify-center">

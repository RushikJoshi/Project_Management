...
import {
  LayoutDashboard, List, BarChart3, Settings2, Plus,
...
export const ProjectDetailPage: React.FC = () => {
  ...
  const onCreateTask = (data: TaskFormData) => {
    const newTask: Task = {
      ...
      order: projectTasks.filter(t => t.status === defaultStatus).length,
          ...
          <div className="flex-1 min-w-0">
            {editingName ? (
              <input
                ...
                autoFocus
                onBlur={e => { updateProject(project.id, { name: e.target.value }); setEditingName(false); }}
              ...
              <div className="space-y-2.5">
                {projectTasks.slice(0, 6).map(task => (
                  <div key={task.id} className="flex items-center gap-2.5 text-xs text-surface-500">
                    <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: STATUS_CONFIG[task.status].color }} />
                    <span className="flex-1 truncate text-surface-700 dark:text-surface-300">{task.title}</span>
                    <span className={cn('badge text-[10px]', STATUS_CONFIG[task.status].bg, STATUS_CONFIG[task.status].text)}>
